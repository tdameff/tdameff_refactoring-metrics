...
//        border7 = BorderFactory.createLineBorder(Color.white, 2);
        ...
        endDate.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent e) {
                ...
                //The smell is: short identifiers
                //'sd' and 'ed' are too short and should be meaningful
                //We can fix this by refactoring the name to something better.
                Date stDate = (Date) startDate.getModel().getValue();
                Date edDate = (Date) endDate.getModel().getValue();				
				if (edDate.before(stDate)) {
                    endDate.getModel().setValue(edDate);
                    edDate = stDate;
                }
				if ((endDateMax != null) && edDate.after(endDateMax.getDate())) {
					endDate.getModel().setValue(endDateMax.getDate());
                    edDate = endDateMax.getDate();
				}
                if ((endDateMin != null) && edDate.before(endDateMin.getDate())) {
                    endDate.getModel().setValue(endDateMin.getDate());
                    edDate = endDateMin.getDate();
                }
				endCalFrame.cal.set(new CalendarDate(edDate));
